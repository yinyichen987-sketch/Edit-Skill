...
SHOTS = [
    ...
    # 第 4 镜：b 段第二段 —— **时长由内容决定，不为凑总长而截断**：
    #   素材里 B-H5「爆能器已部署」在 17.0s、B-H7「获胜」横幅从 21.4s 一直持续到片尾 26.0s。
    #   既然用户说"时长没有硬性要求"，就把「获胜」多留一会儿（26 拍 = 12.1875s），
    #   而不是反过来为凑 16 小节而砍掉结尾。
    ("b", 13.50, [(26, 1.00)], "run_b2",
     "b 段第二段：B-H5(爆能器已部署横幅) → B-H7(**「获胜」横幅，多留一拍半**)"),
]
# 拍数合计 = 16 +（2+5+9）+（3+5）+ 26 = **66 拍 = 30.9375s**。
# ⚠️ **不再是 64 拍 / 30.000s** —— 用户明确说"时长没有硬性要求，不要为了划定死时长
#    而选择奇怪的剪辑手法"。故：**先按内容决定每镜多长，总长是多少就是多少**，
#    再让 BGM 去适配总长（而不是让剪辑去适配 BGM 的小节数）。
#    66 拍不是 4 的整数倍 ⇒ 17 小节（31.875s）的 BGM 会被**裁到 30.9375s**。
...
#   b 13.500 → 25.688  （上界 26.017，余量 0.329s = 约 10 帧，满足 CUT-04）
# 四段互不重叠（SEL-02）。
...
def build(vertical: bool) -> dict:
    ...
    return {
        ...
        "audio_overlays": (
            ...
            [{"source": "../bgm/bgm_128_17bars.wav", "track": "bgm",
              "start": 0.0,
