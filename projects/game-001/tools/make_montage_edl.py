...
#   "Stillness placed against density is what makes density land."
#   ⇒ 第 11 镜是**吸气镜**：取一段本来就低运动的素材（c 4.55，分析里 C-H4
#     「接近过程、无独立音频事件、usable:false」）以 0.5× 播放，做出一个"几乎停住"
#     的拍子，紧接高潮镜。这是本片**唯一的静止点** —— 全片等密度会被读成"平"，
#     而密度只有靠在静止旁边才显得出来。
SHOTS = [
    (2, "c", 6.40,  "hook",    "C-H5 击杀播报同帧、信息量最高的一帧，用最狠的一下开场", 1.0),
    (2, "a", 6.30,  "tension", "A-H3 实测 2 HP（conf 0.95），全集最强悬念", 1.0),
    (1, "a", 11.55, "burst",   "A-H5 右上同时刷出两条击杀播报", 1.0),
    (1, "c", 23.40, "burst",   "C-H11 片尾最后一个高能点，抽出来当短促重音", 1.0),
    (2, "b", 5.40,  "burst",   "B-H1 全 70 秒音频能量最强的一段", 1.0),
    (2, "c", 0.00,  "burst",   "C-H1 近战位移 + 运动模糊，换气", 1.0),
    (4, "b", 8.45,  "bigplay", "B-H2 击杀确认，延展到 B-H3（止于 10.325，避开 10.98 跳变窗）", 1.0),
    (2, "c", 8.90,  "burst",   "C-H7 甩枪光效", 1.0),
    (2, "c", 9.90,  "burst",   "C-H8 交火，正好压在 7.5s 的 drop 上", 1.0),
    (1, "a", 13.40, "burst",   "A-H6 高强度动作（压到 1 拍，给吸气镜让位）", 1.0),
    (1, "c", 4.55,  "gasp",    "**吸气镜**：C-H4 接近过程本身低运动，0.5× 后近乎静止", 0.5),
    (4, "b", 17.00, "climax",  "B-H5 画面有「爆能器已部署」横幅 + 多人交火，吸气之后立刻引爆", 1.0),
    (2, "c", 3.51,  "burst",   "C-H3 跳变之后的一侧（3.4050 + 0.105s，满足 ±0.1s 容差）", 1.0),
    (2, "c", 19.10, "burst",   "C-H10 本段最长的高能量持续段", 1.0),
    (4, "b", 21.40, "ending",  "B-H7「获胜」横幅，收在最强的情绪点上", 1.0),
]
# 吸气镜的素材窗口 = 时间线长度 × 速度 = 0.46875 × 0.5 = 0.2344s（c 4.55–4.784）。
...
#
# ⚠️ 这里**刻意选免费滤镜**，尽管用户已充会员。原因是一条实测/调研得出的硬约束：
#   本机 `User Data\Cache\effect\` 只缓存了 **483 个 effect_id**，而**每一个没用过的
#   VIP 素材都要剪映在首次打开时联网下载**，下载超时会表现为"xx 加载失败"。
#   也就是说"写进草稿"≠"打开就能用"。青橙是免费的（通常已缓存/秒下），保证一次成功；
#   想要更强的电影感可自行在剪映里一键换成 VIP（清单见 spec/剪映导出清单.md）。
GRADE = {"type": "青橙", "intensity": 65.0}

# 转场：只一种（闪白）、只在重音位，0.08s —— 调研结论是"硬切为主，转场只当切点用"
TRANSITION_AT = {1.875, 4.6875, 7.5, 11.25}

# 特效轨（同一时刻的多个特效要分轨，否则 add_segment 的重叠校验直接拒绝）
EFFECTS = [
    ("fx",  "冲击波",   0.0,    0.20),
    ("fx",  "冲击波",   4.6875, 0.20),
    ("fx",  "冲击波",   7.50,   0.25),
    ("fx",  "冲击波",   11.25,  0.25),
    ("fx2", "闪光震动", 9.375,  0.15),   # 吸气之后的高潮落点
    ("fx",  "星火炸开", 13.125, 0.30),
]

# 音效：分两条轨以便同一时刻叠加（drop 处 sub_drop 与 impact 同时响）
# 两条 riser：一条推进到 7.5s 的 drop，一条推进到吸气之后的 9.375s 高潮。
SFX = [
    ("sfx1", "whoosh",    1.875,  0.35, 0.70 * SFX_VOLUME / 0.70),
    ("sfx1", "riser",     5.625,  1.80, 0.55 * SFX_VOLUME / 0.70),
    ("sfx1", "riser",     8.4375, 0.94, 0.60 * SFX_VOLUME / 0.70),
    ("sfx2", "impact_a",  1.875,  0.14, 0.90 * SFX_VOLUME / 0.70),
    ("sfx2", "whoosh",    4.6875, 0.35, 0.70 * SFX_VOLUME / 0.70),
    ("sfx2", "sub_drop",  7.50,   0.90, 0.85 * SFX_VOLUME / 0.70),
    ("sfx2", "impact_b",  9.375,  0.14, 1.00 * SFX_VOLUME / 0.70),
    ("sfx2", "impact_a",  11.25,  0.16, 0.85 * SFX_VOLUME / 0.70),   # impact_a 只有 0.1604s
    ("sfx2", "impact_b",  13.125, 0.16, 0.90 * SFX_VOLUME / 0.70),
]
...
CAPTIONS = [
    ("t1", "别眨眼",       0.0000,  0.9375,  24.0, (1.0, 1.0, 1.0), "弹簧",     "爆闪"),
    ("t2", "2 HP",         0.9375,  0.9375,  24.0, (1.0, 0.35, 0.35), "弹入",   "跳动"),
    ("t3", "播报两连",     1.8750,  0.9375,  20.0, (1.0, 0.9, 0.2),  "弹入",     None),
    ("t4", "听声辨位",     4.6875,  1.40625, 20.0, (1.0, 1.0, 1.0),  "向上滑动", None),
    ("t5", "一枪一个",     7.5000,  1.40625, 22.0, (1.0, 0.9, 0.2),  "复古打字机", "爆闪"),
    ("t6", "还在打",      11.2500,  1.40625, 20.0, (1.0, 1.0, 1.0),  "弹入",     "颤抖"),
    ("t7", "这枪换你，敢不敢上？", 13.1250, 1.8750, 17.0, (1.0, 0.9, 0.2), "渐显", None),
]
...
def build(vertical: bool) -> dict:
    ...
    for i, (beats, key, src_in, role, why, speed) in enumerate(SHOTS, 1):
        ...
        if t in TRANSITION_AT:
            c["transition"] = {"type": "闪白", "duration": 0.08}
        clips.append(c)
        t += dur

    # ---- 关键帧：只给三个 4 拍长镜做推近（克制；且缩放值必须是**绝对**值）----
    #   现在 4 拍长镜是 m07 / m12 / m15（m11 是 1 拍的吸气镜，不长推）
    push = {"m07": 1.28, "m12": 1.30, "m15": 1.26}
    for c in clips:
        end_scale = push.get(c["id"])
        if end_scale:
            base = BAND_SCALE if vertical else 1.0
            c["keyframes"] = [
                {"property": "uniform_scale", "time": 0.0, "value": base},
                {"property": "uniform_scale", "time": c["duration"], "value": round(end_scale, 4)},
            ]
