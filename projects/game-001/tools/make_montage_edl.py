...
SHOTS = [
    # 每个"镜"是 **(素材, 入点, [(拍数, 速度), …], 角色, 依据)**。
    #
    # ⚠️⚠️ **第六版：改成"操作高光"式快切** —— 依据是**六条高播放参考的实测**：
    #   中位镜长 **0.97–2.67s**、切点密度 **20–38 切/分**；
    #   上一版 7 镜 / 均长 4.42s / **12 切每分**，比参考慢 2–3 倍。
    #
    # ★★ **"快切"与"不割裂"如何同时成立**（本轮关键设计，也是唯一自洽的解法）：
    #   用户的要求是「**每个原始素材先后一致**」。故按**轮转交错**排：
    #       a¹ c¹ b¹ a² c² b² a³ c³ b³ a⁴ c⁴ c⁵ b⁴
    #   每段素材内部**只朝时间增大的方向走、绝不回头** ——
    #   既不乱序、也不在同一素材里反复进出。**快，但不割裂。**
    #   （先写过按素材分块的版本 aaaa bbbb ccccc，内部也单调，但读起来是"三条片子拼接"；
    #     交错的观感才像"一条片子"，与参考成片一致。）
    #
    # 参考成片的共性：满屏 16:9 · **无解说字幕**（只有偶发的大号冲击词 ACE/CLUTCH）·
    #   一个常驻角标小窗（本项目没有 facecam 素材，故不做）。
    #   ⇒ 本版**保持零字幕**（用户要求），且**不用变速** —— 1–2 拍的短镜里放不下速度曲线。
    #
    # 时长**不做硬性要求**：总长由这些内容窗口自然决定 = **44 拍 = 20.625s = 11 小节整**。

    # ---- a 段（内部顺序必须递增：0.50 → 6.30 → 11.55 → 13.40）----
    ("a", 0.50,  [(4, 1.00)], "a_open",  "A-H1：开场最强瞬态（band +14.61dB），冷开峰值"),
    ("a", 6.30,  [(2, 1.00)], "a_2hp",   "A-H3：**实测 2 HP**（conf 0.95，全集最高）"),
    ("a", 11.55, [(2, 1.00)], "a_kills", "A-H5：右上**同时刷出两条击杀播报**，且此段音频为全集最高峰值"),
    ("a", 13.40, [(3, 1.00)], "a_fight", "A-H6：双通道同时显著的高强度动作"),

    # ---- b 段（5.40 → 8.45 → 17.00 → 21.40；10.9797/12.9787 两处真实跳变窗全程避开）----
    ("b", 5.40,  [(4, 1.00)], "b_audio", "B-H1：**全 70 秒音频能量最强**的一段"),
    ("b", 8.45,  [(2, 1.00)], "b_kill",  "B-H2：击杀播报确证「我 ⟨枪⟩ ZCQ891」"),
    ("b", 17.00, [(4, 1.00)], "b_plant", "B-H5：画面有「爆能器已部署」横幅 + 多人交火"),
    ("b", 21.40, [(8, 1.00)], "b_win",   "B-H7：**「获胜」横幅**压到片尾 —— 参考成片也都是长镜收尾"),

    # ---- c 段（6.40 → 8.90 → 9.90 → 19.10 → 23.40）----
    ("c", 6.40,  [(3, 1.00)], "c_kill",  "C-H5：击杀播报同帧 + 全集信息量最高的一帧"),
    ("c", 8.90,  [(2, 1.00)], "c_flick", "C-H7：甩枪光效"),
    ("c", 9.90,  [(3, 1.00)], "c_fight", "C-H8：交火（band +18.58dB、flux 25.6）"),
    ("c", 19.10, [(5, 1.00)], "c_long",  "C-H10：本段最长的高能量持续段"),
    ("c", 23.40, [(2, 1.00)], "c_last",  "C-H11：片尾最后一个高能点"),
]

# ★ **交错出场顺序**（应用层按此顺序走 SHOTS，而不是按上面的定义顺序）：
#     a¹ c¹ b¹ a² c² b² a³ c³ b³ a⁴ c⁴ c⁵ b⁴
#   定义顺序按素材分组只是为了**便于人读**；真正的时间线顺序由这张表决定。
SHOT_ORDER = ["a_open", "c_kill", "b_audio", "a_2hp", "c_flick", "b_kill",
              "a_kills", "c_fight", "b_plant", "a_fight", "c_long", "c_last", "b_win"]

# 拍数合计 = a(4+2+2+3=11) + b(4+2+4+8=18) + c(3+2+3+5+2=15) = **44 拍 = 20.625s = 11 小节整**。
# 13 镜 / 12 切 ⇒ **34.9 切每分**（参考区间 20–38 ✓）；
# 镜长中位数 3 拍 = **1.406s**（参考区间 0.97–2.67s ✓）。128BPM ⇒ 1 拍 = 0.46875s。
# ⚠️ 单位是**拍**不是小节（曾把 4/4/2/6 小节数误填进去，总长只剩 7.5s）。
#
# **各段素材内部顺序校验（必须单调递增，否则就是用户说的"割裂"）**：
#   a: 0.50 < 6.30 < 11.55 < 13.40 ✓
#   b: 5.40 < 8.45 < 17.00 < 21.40 ✓
#   c: 6.40 < 8.90 < 9.90 < 19.10 < 23.40 ✓
#   —— 校验脚本：	ools/verify_shot_order.py
#
# **素材窗口上界校验（CUT-04，各留 ≥1 帧余量）**：
#   a 最大出点 13.40+3×0.46875 = 14.806 ≤ 18.984 ✓
#   b 最大出点 21.40+8×0.46875 = 25.150 ≤ 26.017（余 0.867s）✓
#   c 最大出点 23.40+2×0.46875 = 24.3375 ≤ 25.012 ✓
#   各段窗口互不重叠（SEL-02）✓
...
TRANSITION_AT = {3.28125, 10.3125, 16.875}
TRANSITION_LONG_AT = {16.875}
...
EFFECTS = [
    ("fx",  "幻彩故障",  3.28125, 0.22, None),   # 第 3 镜起点
    ("fx",  "像素故障", 10.3125,  0.30, None),   # DROP 后的重音镜起点
    ("fx2", "花屏故障", 16.875,  0.35, None),   # 收尾镜起点（「获胜」）
]
...
SFX = [
    # 总长 20.625s = **11 小节整**；drop 定在第 6 小节 = **9.375s**（前 5 小节建、后 6 小节满）
    ("sfx1", "riser",     7.500, 1.80, 0.65 * SFX_VOLUME / 0.70),   # 推到 9.375 的 drop
    ("sfx1", "impact_b",  9.375, 0.14, 1.00 * SFX_VOLUME / 0.70),
    ("sfx1", "whoosh",    3.28125, 0.35, 0.70 * SFX_VOLUME / 0.70),
    ("sfx2", "sub_drop",  9.375, 0.90, 0.95 * SFX_VOLUME / 0.70),
    ("sfx2", "impact_a", 16.875, 0.16, 0.90 * SFX_VOLUME / 0.70),   # impact_a 只有 0.1604s
    ("sfx2", "win",      17.400, 0.45, 0.85 * SFX_VOLUME / 0.70),   # 「获胜」横幅出现处
    ("sfx2", "whoosh",   10.3125, 0.35, 0.70 * SFX_VOLUME / 0.70),
]
...
def build(vertical: bool) -> dict:
    ...
    seg_no = 0
    # 按 SHOT_ORDER 重排（定义顺序按素材分组只为便于人读）
    by_role = {sh[3]: sh for sh in SHOTS}
    ordered = [by_role[r] for r in SHOT_ORDER]
    assert len(ordered) == len(SHOTS), "SHOT_ORDER 与 SHOTS 数量不一致"
    for shot_i, (key, src_in, parts, role, why) in enumerate(ordered, 1):
    ...
    return {
        ...
        "texts": texts,
        "audio_overlays": (
            # ⚠️ BGM 时长**跟着成片总长走**（而不是让成片去凑 BGM 的小节数）：
            #   用户明确说时长没有硬性要求。BGM 文件做成 17 小节（31.875s）覆盖有余，
            #   这里按**实际总长**引用，末尾 0.25s 淡出收干净。
            [{"source": "../bgm/bgm_128_11bars.wav", "track": "bgm",
              "start": 0.0,
...
def main() -> int:
    print("=== 生成 B站横屏集锦 EDL（128 BPM；总长由内容决定，不再固定）===")
    print("    ⚠️ 用户：时长没有硬性要求 —— 先按内容决定每镜长度，总长是多少就是多少，BGM 去适配它")
    ...
    print("自检口径：切点应全部落在 0.46875s 的整数倍上；"
          "镜长 ≤4s（快切档）；各段素材内部必须**单调递增**（不回头），这是「不割裂」的量化判据。")
    return 0
