...
坐标与字号换算（两类依据分开标注，避免把"设计值"当成"实测值"）：
    ...
    · 字号：剪映 `size` 字段**无公开 px 换算**（`text_segment.py` 只写"字体大小"）。
      因此字框高度取**规格 §4.3 的设计字号**（`[T2 §4.2]`：过程字幕 56px @1080 宽），
      即字框半高 = 56/2 = **28px**。这是**设计值不是实测值**，已在规格 §4.1 声明。

    ⚠️ 若将来拿到剪映真实的 `size → px` 换算，**只需改 `SIZE_PX_BY_KIND`**，
    判定逻辑不变；该缺口已在 `format-spec.md §13` 登记。
...
# 字框高度（px）——**按字号分档**，是设计字号而非实测换算。见模块 docstring。
#   EDL 的 style.size 是剪映内部字号值（钩子/提问 6.0、过程 5.0），
#   按 §4.3 的设计字号档位映射：6.0 -> 72px、5.0 -> 56px。
#   ⚠️ 曾用统一的 28px 半高（= 56/2），那对 72px 行**偏松 8px**，
#   会让 72px 的字形压到边界或越界 —— 由 creative-reviewer 在 t8 期间指出。
SIZE_PX_BY_RAW = {6.0: 72.0, 5.0: 56.0}
...
def half_by_raw(raw_size) -> float:
    """按 EDL 的 style.size 取该行的字框半高（设计字号 / 2）。"""
    return SIZE_PX_BY_RAW.get(raw_size, DEFAULT_SIZE_PX) / 2.0


def main() -> int:
    print("公式: 中心 px = (1 - transform_y) / 2 * canvas_h      （上移为正）")
    print("字框: 中心 ± 半高，半高**按字号分档**（size 6.0→72px→±36；5.0→56px→±28）")
    print("      ⚠️ 半高用设计字号，非剪映实测换算（无公开换算）")
