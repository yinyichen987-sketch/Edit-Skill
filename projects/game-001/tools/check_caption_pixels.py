...
坐标与字号换算（两类依据分开标注，避免把"设计值"当成"实测值"）：
    ...
    · 字号：**该缺口已由实测补上（本轮）**。原写法是"剪映 `size` 无公开 px 换算，
      ...
      两者同时收敛到 **em ≈ 44.4px @ size 8.0** ⇒ **em_px ≈ 5.55 × size**。
...
#   em_px ≈ 5.55 × size，依据：剪映自渲染封面里 size=8.0 → em ≈ 44.4px
...
def half_by_raw(raw_size) -> float:
    ...
    实测换算 em_px ≈ 5.55 × size；字框高按 1 行 = 1 em 计，故半高 = em/2。
    """
...
def main() -> int:
    ...
    print(f"字框: 中心 ± 半高，半高 = 5.55 * size / 2（**实测换算**，非设计值）")
