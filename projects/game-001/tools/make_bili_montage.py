...
SHOTS = [
    # ---- 第一轮：11 条素材各一发 ----
    ("851622c5", 29.430, 4, "检出击杀（本轮最响素材 band −3.58dB 的段内击杀）"),
    ("4c58efb4",  4.670, 4, "检出击杀"),
    ("0a550967",  2.400, 5, "检出击杀（开场冷开峰值附近）"),
    ("b61cc53d",  6.200, 4, "检出击杀"),
    # ⚠️ 原写 0.570 —— 抽帧复核发现那一帧其实是**「购买阶段」横幅**，不是击杀
    #    （顶部同一片区域也会被购买/回合横幅触发 ⇒ 检测器有此类误检）。
    #    换成同一条素材的 4.000（复核通过）。
    ("58b8e283",  4.000, 3, "检出击杀（原 0.570 经抽帧复核为「购买阶段」误检，已换）"),
    ("9526869d",  6.130, 5, "检出击杀"),
    ("7f0d8287",  1.230, 4, "检出击杀"),
    # ⚠️ 原写 4.600（复核时不像击杀）；改用 **6.300** —— 这正是检测器验证过的那个真值
    #    （已知真值 6.35s，检出 6.30s）。
    ("4b0460c4",  6.300, 4, "原 a 段；用**已验证的真击杀** 6.30s（真值 6.35s）"),
    ("38cc4903",  1.300, 4, "检出击杀"),
    ("dcc871b1",  6.700, 4, "原 c 段；对应 C-H5 击杀播报"),
    # ---- 第二轮：5 条最强的再来一发（素材内部时刻仍递增）----
    ("7f0d8287", 13.970, 4, "第二发（该素材检出击杀最多，共 15 个）"),
    ("b61cc53d",  9.570, 4, "第二发"),
    ("58b8e283",  7.200, 4, "第二发"),
    ("38cc4903",  2.900, 4, "第二发"),
    ("0a550967",  6.800, 3, "第二发"),
    # ---- 收尾：b 段，紧邻「获胜」横幅 ----
    ("6b2eea34", 22.830, 4, "原 b 段；**收尾击杀**，其后即为「获胜」横幅"),
]
...
#   0,4,8,13,17,20,25,29,33,37,41,45,49,53,57,60
#    曾写成 {18,40,58}（错拍的版本），18 不对应任何镜起点 ⇒ 那条被**静默忽略**。
TRANSITION_AT_BEAT = {8, 33, 60}        # 第 3 / 9 / 16 镜起点
