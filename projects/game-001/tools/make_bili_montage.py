...
# (素材短名, 击杀帧秒, 拍数, 说明)
SHOTS = [
    # ⚠️ 元组第 2 项是**连杀簇的第一次击杀时刻**（不是入点，也不是单次击杀）。
    #    入点 = 该项 − KILL_LEAD_BEATS 拍，由 build() 统一算。
    #    素材池来自 `analysis2/multikill_clusters.json`：
    #    **2.45s 内至少 2 次击杀**的窗口（11 条素材共 37 簇）。
    #    ⇒ 整段都是交火，避免"跟击杀毫无关系的画面"。
    #
    # 12 镜 / 8×5 拍 + 4×6 拍 = 64 拍 = 16 小节 = 30.0799s；21.9 切每分。
    # 括号里是**该簇内的击杀数**，选段优先取 3 杀的簇。
    #
    # (素材短名, 簇首次击杀秒, 拍数, 说明)
    # ---- 前段：3 杀簇为主，先把密度拉起来 ----
    ("38cc4903",  8.730, 5, "连杀簇 8.73→11.18s（**3 杀**）"),
    ("7f0d8287",  1.230, 5, "连杀簇 1.23→3.68s（2 杀）"),
    # 6 拍：该簇的 3 次击杀在 4.60 / 6.30 / 6.97，5 拍只够到第 1 个 ⇒ 加一拍装进两次
    ("4b0460c4",  4.600, 6, "连杀簇 4.60→7.05s（**3 杀**）；原 a 段"),
    ("dcc871b1", 14.530, 5, "连杀簇 14.53→16.98s（**3 杀**）；原 c 段"),
    ("4c58efb4", 18.800, 5, "连杀簇 18.80→21.25s（**3 杀**）"),
    ("9526869d", 18.800, 5, "连杀簇 18.80→21.25s（**3 杀**）"),
    ("b61cc53d",  9.570, 5, "连杀簇 9.57→12.02s（**3 杀**）"),
    ("58b8e283", 25.530, 5, "连杀簇 25.53→27.98s（**3 杀**）"),
    ("0a550967", 18.300, 5, "连杀簇 18.30→20.75s（**3 杀**）"),
    # ---- 后段：第二发 + 收尾 ----
    ("7f0d8287", 20.370, 6, "连杀簇 20.37→22.82s（**3 杀**）；该素材第二发，时刻递增 ✓"),
    ("851622c5", 29.430, 6, "连杀簇 29.43→31.88s（2 杀）；本轮最响素材 band −3.58dB"),
    ("6b2eea34", 22.830, 6, "连杀簇 22.83→25.28s（2 杀）；原 b 段，**收尾**，其后即「获胜」横幅"),
]
...
#    所以生成时打印命中数量，改完必须核对。
#   12 镜的起点拍号：0,5,10,16,21,26,31,36,41,46,52,58
TRANSITION_AT_BEAT = {10, 41, 58}       # 第 3 / 9 / 12 镜起点
